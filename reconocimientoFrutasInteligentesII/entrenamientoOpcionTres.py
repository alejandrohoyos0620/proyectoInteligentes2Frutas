...
tf.keras.backend.clear_session()  #para restablcer facilmente el estado del portátil
#Red neuronal
...
model.add(Conv2D(kernel_size=7,strides=2,filters=18,padding="same",activation="relu",name="capa_1"))
model.add(MaxPool2D(pool_size=2, strides=2))

model.add(Conv2D(kernel_size=7,strides=2,filters=36,padding="same",activation="relu",name="capa_2"))
model.add(MaxPool2D(pool_size=2, strides=2))
...
model.compile(optimizer="RMSprop", loss="mse", metrics=['accuracy',tf.keras.metrics.Recall(),tf.keras.metrics.Precision()])
...
for train,test in kFold.split(X,y):
    ...
    #print(x[train].shape, y[test].shape)
    model.fit(X[train], y[train], epochs=35,batch_size=200)
...
# Guardar modelo
ruta="models/modeloFrutasTres.h5"
model.save(ruta)
